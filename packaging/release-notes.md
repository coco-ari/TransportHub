# TransportHub v0.1.5（预览版）

这是 Windows 公开预览修复版。TransportHub 在 Syncthing 之上提供一个可置顶、可折叠的
文字和文件投递窗口。

## v0.1.5 修复

- 新增原创 TransportHub 应用图标，以双向传输箭头和中央节点表达组网与文件传输
- EXE、窗口、任务栏、系统托盘、桌面/开始菜单快捷方式、安装器及卸载列表统一使用新图标
- 图标内置 16、20、24、32、40、48、64、128 和 256 像素资源，兼顾不同 DPI 与显示位置

## 此前修复

- 修复窗口刚打开时在线徽标短暂覆盖标题或状态文字的问题
- 修复最后一条消息时间紧贴输入栏时被遮挡的问题
- 标题、发送者、消息正文、附件名称、详情和时间按实际 DPI/字体重新测量
- 为不同电脑的系统字体回退和文字缩放增加少量行高容错，不改变窗口整体尺寸
- 同步已完成但附件确实不存在时显示“已删除”，不再永久停留在“同步中”

## 此前修复

- 修复其他电脑发送大文件时，接收端消息区只显示“同步中”而不显示文件的问题
- 文件元数据到达前先显示临时接收卡片，包括发送电脑、文件名、总大小、百分比和实时速度
- 文件完成并通过完整性校验后，临时卡片自动变为可点击的正式附件

## 此前新增

- 标题状态在同步时实时显示下载和上传速率，空闲后自动恢复精简状态

## 此前修复

- 修复程序运行时卸载后残留进程和可执行文件的问题
- Syncthing 安装或配置失败时中止安装，不再留下半安装状态
- 文件按 SHA-256、目录按清单哈希校验后才显示送达
- 退出时等待传输取消与暂存清理，减少孤儿文件
- 静默升级完成后重新启动 TransportHub
- 再次点击桌面快捷方式会直接唤醒已有窗口
- 粘贴公开图片链接时可下载并作为图片附件发送
- 在 TransportHub 窗口内复制连接码、连接新电脑并确认请求，无需打开 Syncthing 网页

## 本版内容

- 同步文字、链接、文件、文件夹和剪贴板图片
- 拖放发送；点击文字复制；点击附件在资源管理器中定位
- 始终置顶，折叠为屏幕边缘按钮，或隐藏到系统托盘
- 单一 Windows 安装程序，默认创建桌面和开始菜单快捷方式
- TransportHub 在用户登录 Windows 后自动启动
- 若电脑尚未安装 Syncthing，安装程序会通过 WinGet 安装并配置同步文件夹、90 天版本保留、登录自启动和防火墙

## 安装

1. 下载 `TransportHub-Setup-v0.1.5.exe`。
2. 双击安装；防火墙配置时 Windows 可能请求管理员确认。
3. 主电脑点击标题下方的连接状态并复制连接码；新电脑粘贴后点“连接”。
4. 主电脑核对名称后点“接受”，双方会自动共享 `transporthub-data`。

升级时直接运行新版安装程序，无需先卸载。

## 重要说明

- 这是预览版，只应连接由同一个人管理、彼此完全可信的设备。
- 安装器目前未做代码签名，Windows 可能显示 SmartScreen 或“未知发布者”。请仅从本仓库 Release 下载，并使用 `SHA256SUMS.txt` 核验。
- 安装 Syncthing 需要网络及 Windows App Installer/WinGet；Syncthing 二进制不包含在本安装器内。
- 卸载 TransportHub 会保留 Syncthing、设备身份、同步配置和同步数据，避免误删。同步不是备份。
- 仅支持 Windows 10/11；暂无自动更新与实时音视频。
